...
### 12:26 [Mac mini] 先進光/禾伸堂建倉 + 台光電換股 — 實際成交入帳 + 撞到永豐每日投資上限 + 現金結構修正 + Kelly p/b 討論
- **觸發**：Sir 問「先進光買了嗎」發現沒有→查 DB 沒有紀錄，接續決定買先進光（≤5%）+ 禾伸堂加碼到 10%，資金來源台光電 exit 換股。過程中反覆調整分批計畫（3批→2批、股數配比），最後發現實際成交都在永豐 App 截圖、直接以截圖為準記帳，計畫下單只是過程、不是最終真相
- **實際成交入帳**（永豐證委託回報截圖 IMG_9901/9902.PNG 逐筆核對，跟 `order add` 計畫下單脫鉤、直接 `record_transaction`）：
  - 台光電 2383.TW 賣出 6,000 股（3,000@5465、3,000@5410），原計畫 9,000 股只完成 2/3，剩 3,000 股沒賣
  - 先進光 3362.TWO 買進 45,000 股（177/177/177/178.5/176/179/179.5 七筆不同價），原計畫 90,000 股只完成一半，被永豐擋住
  - 禾伸堂 3026.TW 買進 21,000 股（11,000@1035、10,000@1025），加上 7/7 已有的 7,000 股，現在共 28,000 股
- **重大發現：永豐「客戶投資上限」= 每日 NT$3,000萬、只算買進金額、賣出不計入**——兩筆先進光失敗委託單的錯誤訊息（「可用餘額367500元」「可用餘額13290000元」）跟累計買進金額精確吻合（誤差 0），證實這不是模糊的風控概念、是精確算好的硬性額度。Sir 原本猜「3,000萬」被我一開始誤判成別的機制，後來用實際失敗委託單的錯誤訊息反推驗證，Sir 是對的
- **現金結構修正**：發現永豐證（account_id=3）本身不持有現金、交割走永豐銀（account_id=5）——`tx buy` 的雙重記帳邏輯誤在永豐證造出負的現金部位，用截圖裡查到的永豐銀真實餘額（NT$942,100）直接校正兩個帳戶。這條規則寫進 `~/.claude/skills/pdb-sync/SKILL.md`（hub git repo，不在 portfolio-db repo 內）避免以後再犯
- **投資哲學討論（純討論、無程式碼/資料異動）**：
  - 現金緩衝：從「習慣高水位、不留緩衝」延伸到「長期持有股票會賺錢，為什麼要留現金」——結論現金的價值不是報酬、是選擇權（today 先進光額度撞牆就是活教材）跟過年封閉市場那幾天的流動性缺口（T+2 在休市期間失效，不是價格問題是市場根本沒開）；緊急需求金額對齊 Sir 過去實際經驗（從未發生過臨時需要幾百萬），過年那個特定情境用「有價證券質押借款」（提前申請的額度、不用賣股票、不受休市影響）解掉、優於單純留現金
